...
*   **v2 API DTOs** (2025-10-03): API request and response DTOs are consistently implemented as records in `api/dto/request/` and `api/dto/response/`, with validation annotations (e.g., `@NotNull`, `@Min`, `@Max`). Recent addition: `EventRegisterRequest` record in `api/dto/request/` with optional `amountOfParticipants` field (int, default 1, validated 1-3) for the event registration endpoint, ensuring backward compatibility and type-safe data transfer.
*   **API Docs** (Updated: 2025-10-03): New endpoints will appear in OpenAPI when dev profile is enabled; verify schemas for Map<Integer, Integer> serialization in UserStatsResponse.
